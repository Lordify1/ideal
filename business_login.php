...
<nav class="navbar majornav navbar-expand-sm navbar-dark   
    fixed-top animate__animated animate__fadeIn">
...
if(isset($_SESSION["userfeedback"])){
  echo "<div class='alert alert-info'>" . $_SESSION["userfeedback"] . "</div>";
  unset($_SESSION["userfeedback"]);
}


if(isset($_SESSION["blocked"])){
  echo "<div class='alert alert-info'>" . $_SESSION["blocked"] . "</div>";
...
?>
  <form action="process/business_login.php" method="post">
    ...
    <input type="hidden" name="login_time" id="login_time" value="<?php echo date("Y") . " " . time() ?>">
    <div class="form-floating">
    ...
    <input type="submit" name="signin" value="Sign In" class="btn btn-primary w-100 py-2 my-1" id="signin">
    <p class="mt-2 mb-3 text-body-secondary">&copy; iDEAL <?php echo date("Y") ?></p>


  </form>
</main>
<footer class="container position-fixed bottom-0">
  <div class="row loginfooter p-3">
  <div class="col">
  <span id="donthaveanaccount">Don't have an account? </span><a href="business_register.php" id="registerhere" style="text-decoration: none;"><span>Register here</span></a>

...
<div class="accordion-item">
      ...
      ?>
        <tr>
            ...
            <div class="btn-group">
              ...
              <button class="fa fa-face-frown btn btn-danger" name="cancel"></button>
            </div>
            </td>
            </form>
        </tr>
  ...
  <div class="accordion-item">
    <h2 class="accordion-header" id="completed">
      <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapsecompleted" aria-expanded="false" aria-controls="collapsecompleted">
          Completed Payments
          &nbsp;
      <?php $data = $payment->completed_payments();
  ...
  <div class="accordion-item">
    <h2 class="accordion-header" id="cancelled">
      <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapsecancelled" aria-expanded="false" aria-controls="collapsecancelled">
          Cancelled Payments
          &nbsp;
      <?php $data = $payment->cancelled_payments();
    ...
    <div id="collapsecancelled" class="accordion-collapse collapse" aria-labelledby="cancelled" data-bs-parent="#accordionExample">
      <div class="accordion-body">
  <table class="table table-striped table-success border-white">
    <thead>
        <th>S/N</th>
    ...
    <tbody>
      <?php $sn = 1;
        foreach($cancelled_payments as $cancel){     
      ?>
        <tr>
            <td><?php echo $sn++ ?></td>
            <td>&#8358;<?php echo number_format($cancel["pp_amt"]) ?>.00</td>
            <td><?php echo $cancel["payment_date"] ?></td>
            <td><?php echo $cancel["business_email"] ?></td>
            <td><?php echo $cancel["project_title"] ?></td>
            <form action="process/payment.php" method="post">
            <td>
              <div class="btn-group">
              <input type="hidden" name="ref" value="<?php echo $cancel["transaction_id"] ?>">
                <button class="fa fa-refresh btn btn-primary" name="refund_business"></button>
              </div>
            </td>
            </form>
        </tr>
